		...
		<div class="row">
			<div class="col-lg-8">
				<div class="row">
					<div class="col-lg-9">
						<h1>
							<spring:message code="label.user.my_questions_likes" />
						</h1>
					</div>
				</div>
				<br>
				<c:if test="${!empty questionLikes}">
					<c:forEach items="${questionLikes}" var="question">
						<div class="row">
							<div class="col-lg-12">
								<i> <span class="glyphicon glyphicon-user"></span> &nbsp; <c:out
										value="${question.getQuestion().getUser().username}" />
								</i>
								<sec:authorize
									access="hasRole('Admin') or '${authUsername}'=='${question.getQuestion().getUser().username }'">
									<a href="<c:url value='/questions/delete?id=${question.id}' />"
										class="pull-right" role="button"> <span
										class="glyphicon glyphicon-remove"></span></a>
									<a
										href="<c:url value='/questions/edit?id=${question.getQuestion().id}' />"
								...
								<h2>
									<a
										href="<c:url value='/questions/${question.getQuestion().id}' />">
										<c:out value="${question.getQuestion().title}" />
									</a>
								</h2>
								...
								<i><span class="glyphicon glyphicon-time"></span> &nbsp; <fmt:formatDate
										...
										value="${question.getQuestion().created_at}"
										pattern="d MMMM yyyy, HH:mm" /> </i>

			<div class="pull-right">
										<a class="glyphicon glyphicon-comment comment" 
										href="<c:url value='/questions/${question.getQuestion().id}#answer' />">${fn:length(question.getQuestion().getAnswers())}</a>
